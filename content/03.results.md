...
All data available on the Portal was uniformly processed using `scpca-nf`[@url:https://github.com/AlexsLemonade/scpca-nf], an open-source and efficient Nextflow[@url:https://www.nextflow.io/docs/latest/index.html] workflow for quantifying single-cell and single-nuclei RNA-seq data. 
...
The results from all analyses we conducted comparing Cell Ranger to `alevin-fry` are available in a public analysis repository, `alsf-scpca`[@url:https://github.com/AlexsLemonade/alsf-scpca]. 
...
Then, low-quality cells are identified and removed with `miQC` [@doi: 10.1371/journal.pcbi.1009290], a data-driven approach to identifying low-quality cells.
`miQC` jointly models the proportion of mitochondrial reads and detected genes per cell and calculates a probability that each cell is compromised. 
Cells likely to be compromised (probability > 0.75) are flagged as low-quality cells and removed.
