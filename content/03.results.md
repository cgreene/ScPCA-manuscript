...
To identify the most appropriate reference to use with `SingleR`, we annotated a handful of samples across multiple disease types with all human-specific references available in the `celldex` package [@doi:10.18129/B9.bioc.celldex; @doi:10.1038/s41590-018-0276-y].
...
We found that the `BlueprintEncodeData` [@doi:10.3324/haematol.2013.094243; @doi:10.1038/nature11247] reference, which includes a variety of normal cell types and provides both the human-readable cell name and cell ontology identifier [@url:https://www.ebi.ac.uk/ols4/ontologies/cl], either had the highest or equivalent delta median distribution across samples from different disease types (Supplemental Figure 4).
...
If `CellAssign` is unable to find a likely cell type from the marker gene matrix, a cell type is not assigned. 
Because we are annotating cells from tumor samples using references containing only normal cells, we anticipate that many cells, particularly the tumor cells, may not have an exact match, something we want the user to be aware of.
Cell types were assigned using `CellAssign` on a handful of samples and a large portion of the cells were unassigned, an example of which is shown in Supplemental Figure 5A where unassigned cell types are labeled with `Unknown`. 
When comparing annotations obtained from `CellAssign` to submitter-provided annotations, we noticed the labels for non-tumor cells are similar between `CellAssign` and submitter annotations, while the tumor cells are unable to be assigned using `CellAssign` (Supplemental Figure 5B). 
