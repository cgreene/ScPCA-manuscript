...
For some samples, submitters included data from additional sequencing modalities, including corresponding ADT or CITE-seq data, multiplexing using cell hashing, spatial transcriptomics, or bulk RNA-seq. 
To make all received data available, we included additional modules in `scpca-nf` to perform uniform processing of these additional sequencing modalities.
With the modularity of Nextflow, we were able to develop `scpca-nf` so that it can process sequencing data from a mix of sequencing types. 
This means that libraries with and without ADT data or any of the modalities discussed here can be processed together in a single run. 
Nextflow will handle the parallel processing and ensure that libraries from each sequencing type are run through the correct modules for that sequencing type.
...
To process these libraries, both FASTQ from single-cell/single-nuclei RNA-seq and CITE-seq were provided as input into `scpca-nf` and quantified using `salmon alevin` and `alevin-fry` (Supplemental Figure 2A).
...
This file is also input to `scpca-nf` and used to build the index for quantification of the ADT FASTQ and creation of the cell by ADT matrix.
Unlike with RNA counts, we do not perform any filtering of cells due to low-quality ADT expression. 
However, we do include the results from running `DropletUtils::cleanTagCounts()` on the ADT counts matrix in both the filtered and processed objects produced by `scpca-nf`.
Similar to RNA counts, we normalize ADT data and provide the normalized counts matrix in the processed SCE object. 
...
Instead, we include a column in the `colData` of the processed SCE object that indicates if it is recommended to remove cells due to low expression of ADTs.
...
The ADTs with the most variable expression are also identified. 
The report includes both UMAPs and density plots highlighting the 4 most variable ADTs in the data. 
...
Similar to ADT data, if any libraries were multiplexed and have an associated cell hashing library, both the RNA and HTO FASTQ are provided as input to the workflow and quantified with `salmon alevin` and `alevin-fry` (Supplemental Figure 2C). 
...
The genetic demultiplexing used in `scpca-nf` uses the method described in Weber et al [@doi:], which utilizes matching bulk RNA-seq data from the same sample as genotype data.
Variants among the samples within each pool are identified from the mapped bulk RNA-seq data and pooled single-cell or single-nuclei RNA-seq reads. 
Variants from both bulk and single-cell or single-nuclei RNA-seq are genotyped and used to call the sample of origin with `vireo` [@doi:].
...
The results from these three methods are included in the filtered and processed objects on the Portal. 
If a sample does not include associated bulk RNA-seq data then no genetic demultiplexing results will be included in the objects. 
...
For some samples, multiple libraries were collected with the additional libraries being used for  RNA-seq and/or spatial transcriptomics. 
Both of these additional sequencing methods are supported by `scpca-nf`. 
FASTQ from bulk RNA-seq are first trimmed using `fastp` and then aligned using `salmon` (Supplemental Figure 3A). 
The bulk module of `scpca-nf` outputs a single tsv file with the sample by gene matrix for all samples in a given project.
This sample by gene matrix is included only with project downloads on the Portal. 
As there is not yet support for spatial transcriptomics with `alevin-fry`, `scpca-nf` uses `spaceranger` to quantify all data from spatial transcriptomics.
The required input includes the RNA FASTQ and slide image. 
The output includes the spot-by-gene matrix along with a summary report produced by the `spaceranger`.
