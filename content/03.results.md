...
<!-- JAS: I tend to think we should remove the file names and extensions from the Results: they make sense to me in methods but seem a bit deep in the weeds for this section. Just the object types I think is probably sufficient -->
...
A UMAP plot with cells colored by the total number of genes detected and a faceted UMAP plot where cells are colored by the expression of a top highly variable gene are also provided (Figure {@fig:fig2}F-G).
...
Some samples also included libraries used for bulk RNA-seq and/or spatial transcriptomics.
...
<!-- Deleted phrase about difficulty: we _could_ do this without too much trouble; the challenge for now was presenting it without making it likely to be misused -->
