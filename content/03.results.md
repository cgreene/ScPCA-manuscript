...
If downloading data from a ScPCA project as a single, merged file, the download will include a single `.rds` or `.hdf5` file, a summary report for the merged object, and a folder with all individual QC and cell type reports for each library found in the merged object (Figure {@fig:fig3}B).
...
An example of this UMAP showing a subset of libraries from a ScPCA project is available in Figure {@fig:fig3}D. 
