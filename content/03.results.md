...
When downloading data for an entire project, users can download the individual files for each sample (default) or one file containing the gene expression data and metadata for all samples. 
Users also have the option to receive the data as `SingleCellExperiment` (`.rds`) or `AnnData` (`.hdf5`) objects.
...
These objects house the summarized gene expression data and associated metadata for each library. 

Providing all data from all libraries within a single file makes it easier for users to perform joint analysis on multiple samples simultaneously. 
...
This merged object contains a singular object with all raw and normalized gene expression data and metadata for all single-cell and single-nuclei RNA-seq libraries within a given ScPCA project.
The data present in the merged object has simply been combined and no batch-corrected or integrated data is included. 
...
Regardless of downloading all samples in a project as individual files or as a merged object, the download will include a `single_cell_metadata.tsv`, containing relevant metadata for all samples, and a `README.md`.  
...
To create the merged objects, we created an additional stand-alone workflow for merging the output from `scpca-nf`, `merge.nf` (Figure 3C).
`merge.nf` takes as input the processed `SingleCellExperiment` objects output by `scpca-nf` for all single-cell and single-nuclei libraries included in the ScPCA project.
...
All objects on the Portal were quantified using the same index with the same set of genes, so the genes found in the merged object will be the same genes found in each individual object.
Any metadata found in the individual processed objects will also be merged (e.g., `colData`, `rowData`, and `metadata`). 
After merging all data, high-variance genes are selected in a library-aware manner prior to performing dimensionality reduction with both PCA and UMAP on the combined normalized counts matrix. 
...
If any libraries in a project are multiplexed, the HTO data is not merged and will not be included in the merged object. 

All merged `SingleCellExperiment` objects are then converted to `AnnData` objects and exported as `.hdf5` files. 
If the merged object contains an `altExp` with merged ADT data, the RNA and ADT data will be separated prior to converting to `AnnData` objects to create a `_rna.hdf5` and `_adt.hdf5` file. 

`merge.nf` outputs a summary report for each merged object. 
This report includes a brief summary of the types of samples and libraries included in the portal. 
Tables showing the total number of samples for each diagnosis and the number of libraries from each library type represented in the project are included. 
...
An example of this UMAP showing a subset of libraries from an ScPCA project is available in Figure 3D. 
