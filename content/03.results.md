...
Most samples found on the Portal were obtained from patients with leukemia (n = TODO). 
The Portal also includes samples from brain and central nervous system tumors (n = TODO), sarcoma and soft tissue tumors (n = TODO), and a variety of other solid tumors (n = TODO). 
Most samples were collected at initial diagnosis (n = TODO), with a smaller number of samples collected either at recurrence (n = TODO), during progressive disease (n = TODO), or post-mortem (n = TODO). 
Along with the patient tumors, the Portal contains a small number of human tumor cell line samples (n = TODO).
