...
The `SingleR` output of cell type annotations and a score matrix for each cell and each possible cell type were added to the processed `SingleCellExperiment` object output. 
As a measure of the reliability of the `SingleR` cell type assignments, we also calculated a delta median statistic for each cell by subtracting the median cell type score from the maximum score for that cell. 
