...
The submitted metadata was standardized across projects, including converting all ages to years, removing abbreviations used in diagnosis, subdiagnosis, or tissue of origin, and using standard terms across projects as much as possible for diagnosis, subdiagnosis, disease timing, and tissue of origin.
For example, all samples obtained at diagnosis were assigned the term `Initial diagnosis` for disease timing.

In an effort to ensure sample metadata for ScPCA is compatibile with CZI's CELLxGENE, ontology term identifiers were assigned to the following metadata categories for each sample following the guidelines present in the CELLxGENE schema [@url:https://cellxgene.cziscience.com/;@url:https://github.com/chanzuckerberg/single-cell-curation/blob/main/schema/3.0.0/schema.md]:


| Metadata    | Ontology term description             |
| ----------- | ------------------------------------------------------------------------------ |
