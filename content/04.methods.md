...
To quantify gene expression data for spatial transcriptomics libraries, we used 10x Genomics' Space Ranger [@url:https://www.10xgenomics.com/support/software/space-ranger/latest]. 
...
`fastp` was used to trim adaptors and perform quality and length filtering on all reads present in FASTQ files from bulk RNA-seq. 
...
The top 2000 shared high-variance genes are identified from the merged counts matrix by modeling variance separately for each library.
