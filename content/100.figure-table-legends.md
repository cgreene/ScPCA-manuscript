...
<!-- Table S2 -->
**Table S2. Summary of references used for CellAssign.**
This table provides a summary of the references used for assigning cell types for fScPCA projects using `CellAssign`. 
All references were built by selecting all cell types from a specified set of organs present in the marker gene list from `PanglaoDB`. 
