...
Third, the summary QC report will additionally include a `CITE-seq` section with additional information about ADT-level processing.
Fourth, the workflow exports `SCE` objects with both RNA and ADT results present, but the workflow exports separate `AnnData` files for RNA and ADT results each.
...
Cells shown in this panel are the same cells denoted "Remove" in the corresponding QC report figure represented in Figure {@fig:fig2}E (although a different library is shown).
...
First, a TSV file providing information about library pools is also are inputted to `alevin-fry` along with the RNA FASTQ file.
...
<!-- Figure S3 -->
![**Processing other modalities `scpca-nf`.**](https://raw.githubusercontent.com/AlexsLemonade/scpca-paper-figures/main/figures/compiled_figures/pngs/figure_s3.png?sanitize=true){#fig:figs3 tag="S3" width="7in"}

A. Overview of the bulk RNA-Seq workflow.
A set of FASTQ files is provided as input.
Reads are trimmed, and `salmon` is used to map reads and quantify counts.
FASTQ files are then grouped by ScPCA Project ID, and a sample-by-gene count matrix in TSV format is exported for each Project.

B. Overview of the spatial transcriptomics workflow.
The FASTQ file and tissue image for a given sample are provided as input to `Space Ranger`.
The workflow directly returns the results from running `Space Ranger` without any further processing.

<br><br>

<!-- Figure S4 -->
![**Example plots as shown in the cell type annotation summary report assessing annotation quality.**](https://raw.githubusercontent.com/AlexsLemonade/scpca-paper-figures/main/figures/compiled_figures/pngs/figure_s4.png?sanitize=true){#fig:figs4 tag="S4" width="7in"}

A. Diagnostic plot showing `SingleR` cell type annotation quality, shown for library `SCPCL000001`.
...
Each black point represents a cell, where closed circles denote cells with low-quality annotations and open circles denote cells with low-quality annotations, as assessed by `SingleR`.
...
B. Diagnostic plot showing `CellAssign` cell type annotation quality, shown for library `SCPCL000001`.
...
Each row displays probabilities for only the cells associated with the given cell type annotation, and blue line segments represent a "rug" showing the probabilities for individual cells in each distribution.
...
<!-- Figure S5 -->
![**Example heatmap as shown in the cell type annotation summary report comparing annotations across methods.**](https://raw.githubusercontent.com/AlexsLemonade/scpca-paper-figures/main/figures/compiled_figures/pngs/figure_s5.png?sanitize=true){#fig:figs5 tag="S5" width="7in"}

This example heatmap displays submitter-provided annotations to annotations with `SingleR` and `CellAssign`, shown for library `SCPCL000498`.
