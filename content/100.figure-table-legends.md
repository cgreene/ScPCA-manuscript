...
![**Figure S1. Results from benchmarking `alevin-fry` and `CellRanger` performance.**](https://raw.githubusercontent.com/AlexsLemonade/scpca-paper-figures/main/figures/compiled_figures/pngs/figure_s1.png?sanitize=true){#fig:figs3 width="7in"}

Each panel compares metrics for six representative ScPCA libraries, including three single-cell and three single-nuclei suspensions, obtained when processing data with both `alevin-fry` and `CellRanger`.
Panels B-D consider only cells present in both the `alevin-fry` and `CellRanger` output.
