...
B. Points are ranked by the total UMI count for each droplet. The color in this plot indicates the percentage of droplets that pass filtering of empty droplets. These cells are obtained in the `Filtered SCE Object`.
C. The number of genes detected for each cell passing the cell filter against the total UMI count, where points are colored by the percentage of mitochondrial reads in the cell.
...
The `miQC` model diagnostic plot is only present in summary QC reports whose libraries were filtered with `miQC`.
